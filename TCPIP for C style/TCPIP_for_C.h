#pragma once
//This one is develop TCPIP in c file or project 
...
inline int  Start_TCP_Server(SOCKET* psockfd, WORD Port, DWORD EVENT, HWND Hwnd)
{
	WSADATA  wsadata;
	int      err;
...
inline int Start_TCP_Client(SOCKET* psockfd, WORD R_Port, char* IP, DWORD EVENT, HWND Hwnd) {
	WSADATA  wsadata;
	int      err, i, on = 1;
...
inline int Start_UDP_Server(SOCKET* psockfd, WORD Port, int EVENT, HWND Hwnd) {
	WSADATA  wsadata;
	int      err;
...
inline int Start_UDP_Client(SOCKET* psockfd, sockaddr_in* udpclient, WORD R_Port, char* IP, int EVENT, HWND Hwnd) {
	WSADATA     wsadata;
	int         err, i;
	LPHOSTENT   HostIP;
	char      IP11[100];
...
inline int wstrcpy(wchar_t* Dst, int Size, wchar_t* Src) {
	int i, Len;
	Len = wstrlen(Src, Size); if (Len > Size - 1) Len = Size - 1;
	for (i = 0; i <= Len; i++) Dst[i] = Src[i];
	return Len;
}
inline int wstrcat(wchar_t* Dst, int Size, wchar_t* Src) {
	int i, Len1, Len2;
...
inline int Cmd_Proc(char* S1, char* Command, char Flag, int Command_Num, int Command_Len) {
	int  i, j, k1, k2;
	unsigned char* US1;
	...
	for (i = k1 + 1; i < Command_Num; i++) { Command[i * Command_Len + 0] = 0; }

	return (((k1 == 0) || (k2 == (Command_Len - 1))) ? -1 : k1);
}